...
/* ══════════════════════════════════════════════════════
   AI REVIEW PAGE — requires /ai-review/:projectId
══════════════════════════════════════════════════════ */
const AiReviewPage = () => {
  ...
  // Redirect to projects list if no projectId provided
  useEffect(() => {
    if (!projectId) navigate('/projects', { replace: true });
  }, [projectId, navigate]);

  const { loadingMap, errorMap, resultsMap, executeAgent } = useAgents();
  const isProcessing = loadingMap['review'] || false;
  const error = errorMap['review'] || null;
  const result = resultsMap['review'] || null;
  ...
  const handleRunAi = async () => {
    if (!projectId) return;
    try {
      await executeAgent('review', projectId);
    } catch {
      // error already stored in errorMap
    }
  };
      ...
      setMessages(prev => [
        ...prev,
        {
          ...
          text: typeof result === 'string'
            ? result
            : (result?.data?.message || result.message || "AI Analysis complete. Project insights updated."),
          timelineUpdate: result?.data?.timeline || result?.timeline
        }
      ]);
    }
  }, [result]);
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: '0.85rem', opacity: 0.8 }}>
            Continuous audit and optimization for project <code style={{ background: 'rgba(255,255,255,0.15)', padding: '0.1rem 0.4rem', borderRadius: 4 }}>{projectId}</code>
          </p>
        </div>
        <button
          onClick={handleRunAi}
      ...
      <div style={chatContainer}>
        ...
        <div style={chatSection}>
          <div style={{ flex: 1, padding: '1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
              ...
              <div key={msg.id} style={messageBubble(msg.sender)}>
                ...
                {msg.suggestionAction && (
                  <div style={{
                    ...
                    fontSize: '0.82rem', border: '1px solid rgba(255,255,255,0.2)'
                  }}>
              ...
              <div style={messageBubble('ai')}>
                <div style={{ display: 'flex', gap: '4px' }}>
                  {[0, 0.2, 0.4].map((delay, i) => (
                    <div key={i} style={{ width: 6, height: 6, background: '#fff', borderRadius: '50%', animation: `chatPulse 1s infinite ${delay}s` }} />
                  ))}
                </div>
              </div>
            )}
            ...
            {error && (
              <div style={{ color: '#ef4444', textAlign: 'center', fontSize: '0.82rem', padding: '1rem' }}>
                ⚠️ Error: {error}
              </div>
            )}
          </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <input
                ...
                style={{
                  ...
                  color: isDark ? '#fff' : '#1e293b', fontSize: '0.9rem', outline: 'none'
                }}
              />
              <button style={{
